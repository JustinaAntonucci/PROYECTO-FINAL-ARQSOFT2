...
const HomePage = () => {
  ...
  const getHotels = async () => {
    try {
      const request = await fetch("http://localhost:8082/search-api/searchAll=*:*");
  ...
  const filterHotels = async () => {
    ...
    if (isEmpty(city)) {
      ...
      if (!endDateValue || !startDateValue) {
        request = await fetch(`http://localhost:8082/search-api/search=city_${cityInLowerCase}`);
      } else {
        request = await fetch(`http://localhost:8082/search-api/search=city_${cityInLowerCase}_${startDate}_${endDate}`);
      }
      const response = await request.json();
      if (response !== null) {
