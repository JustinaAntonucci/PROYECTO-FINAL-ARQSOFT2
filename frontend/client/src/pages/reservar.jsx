...
const ReservaPage = () => {
  ...
  const handleReserva = async (e) => {
    ...
    if (auth.userType === false) {
      ...
      fetch('http://localhost:8081/user-res-api/booking', {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    if (hotelId) {
      fetch(`http://localhost:8080/hotels-api/hotels/${hotelId}`)
  ...
  const filterHotels = async () => {
    ...
    const request = await fetch(`http://localhost:8081/user-res-api/hotel/availability/${hotelId}/${startDateParsed}/${endDateParsed}`);
