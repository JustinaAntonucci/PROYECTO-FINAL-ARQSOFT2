...
const HomePage = () => {
  ...
  const getReservations = useCallback(async () => {
    if (auth.accessToken && auth.userType === false) { // userType === false significa que es un cliente
      ...
      try {
        const request = await fetch(`http://localhost:8081/user-res-api/booking/user/${accountId}`);
  ...
  const handleDeleteReserva =  async (bookingId) => {
    ...
    try {
      const response =  await fetch(`http://localhost:8081/user-res-api/booking/${bookingId}`, {
        method: 'DELETE',
        headers: {
